...
<div class="space-y-6">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            ...
            <p class="text-sm text-slate-500 mt-0.5">Perbarui data <span class="font-semibold text-slate-700">{{ $pegawai->nama }}</span></p>
        </div>
    </div>
        ...
        <form action="{{ route('admin.pegawai.update', $pegawai->id) }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <select id="pool_id"
                    ...
                    <option value="{{ $pool->id }}" {{ old('pool_id', $pegawai->pool_id) == $pool->id ? 'selected' : '' }}>
                        {{ $pool->nama_pool }}
                    </option>
                    @endforeach
                </select>
