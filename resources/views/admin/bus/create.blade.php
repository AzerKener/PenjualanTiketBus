...
<div class="space-y-6">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            <h1 class="text-2xl font-bold text-slate-800">Tambah Bus</h1>
            <p class="text-sm text-slate-500 mt-0.5">Daftarkan armada bus baru</p>
        </div>
    </div>
        ...
        <form action="{{ route('admin.bus.store') }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <select id="tipe_bus"
                    ...
                    <option value="Ekonomi" {{ old('tipe_bus') == 'Ekonomi' ? 'selected' : '' }}>Ekonomi</option>
                    <option value="VIP" {{ old('tipe_bus') == 'VIP' ? 'selected' : '' }}>VIP</option>
                    <option value="Executive" {{ old('tipe_bus') == 'Executive' ? 'selected' : '' }}>Executive</option>
                </select>
                @error('tipe_bus')
                <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                @enderror
