...
@section('content')
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8"
        ...
        <div class="flex items-center gap-2 mb-8">
            ...
            <div class="flex items-center gap-2">
                <div :class="step === 2 ? 'bg-blue-600 text-white' : (step > 2 ? 'bg-green-500 text-white' :
                    'bg-slate-200 text-slate-500')"
                    class="w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-colors">2
                </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
                    ...
                    <div class="flex justify-end mb-3">
                        <div
                            class="bg-slate-100 rounded-lg px-3 py-1.5 text-xs text-slate-500 font-medium flex items-center gap-1.5">
...
=======
                                ...
                                <div class="contents">
                                    ...
                                    <button type="button"
                                        :class="{
                                            ...
                                            'bg-slate-100 text-slate-400 cursor-not-allowed border-slate-200': terisi
                                                .includes(kursi),
                                            'bg-white border-slate-300 hover:border-blue-400 hover:bg-blue-50 text-slate-700 cursor-pointer':
                                                !dipilih.includes(kursi) && !terisi.includes(kursi)
                                        }"
                                        class="h-10 rounded-lg border-2 text-xs font-bold transition-all duration-150"
                                        :disabled="terisi.includes(kursi)" @click="toggleKursi(kursi)" x-text="kursi">
                                    </button>
                                </div>
                            </template>
                        </div>
                    ...
                    <div x-show="dipilih.length > 0" x-cloak class="mt-4 p-3 bg-blue-50 rounded-xl border border-blue-100">
                        <p class="text-xs text-blue-600 font-semibold mb-1">Kursi dipilih (<span
                                x-text="dipilih.length"></span>):</p>
                        <div class="flex flex-wrap gap-1.5">
                            <template x-for="k in dipilih" :key="'tag-' + k">
                                <span class="px-2 py-0.5 bg-blue-600 text-white rounded text-xs font-mono font-bold"
                                    x-text="k"></span>
                            </template>
                        </div>
                    </div>
                    ...
                    <div class="mt-5 border-t border-slate-100 pt-5">
                        <label class="flex items-start gap-3 cursor-pointer group">
                            ...
                            <div>
                                <p class="font-semibold text-slate-700 group-hover:text-blue-600 transition-colors text-sm">
                                    Tiket Pulang-Pergi</p>
                        ...
                        <div x-show="isRoundTrip" x-cloak class="mt-3 space-y-3">
                            <div>
                                <label class="block text-xs font-semibold text-slate-600 mb-1.5">Pilih Jadwal
                                    Pulang</label>
                                <select x-model="jadwalPulangId" @change="loadKursiPulang($event.target.value)"
                                    ...
                                    @if ($jadwalPulangList->isEmpty())
                                        <option value="" disabled selected>-- Tidak ada jadwal pulang yang tersedia
                                            --</option>
                                    @else
                                ...
                                <div class="grid gap-1.5 overflow-y-auto max-h-52"
                                    ...
                                    <template x-for="(kursi, idx) in semuaKursiPulang" :key="'p-' + kursi">
                                        <div class="contents">
                                            ...
                                            <button type="button"
                                                :class="{
                                                    'bg-purple-600 text-white border-purple-600': dipilihPulang
                                                        .includes(kursi),
                                                    'bg-slate-100 text-slate-400 cursor-not-allowed border-slate-200': terisiPulang
                                                        .includes(kursi),
                                                    'bg-white border-slate-300 hover:border-purple-400 hover:bg-purple-50 text-slate-700':
                                                        !dipilihPulang.includes(kursi) && !terisiPulang.includes(kursi)
                                                }"
...
>>>>>>> af1af08bd08d97c03e17c1b82e23ab797d271407
                ...
                <div class="space-y-4">
                    ...
                    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
                        ...
                        {{-- Info kursi dipilih --}}
                        <div x-show="dipilih.length > 0" x-cloak
                            ...
                            <div class="flex flex-wrap gap-1">
                                <template x-for="kursi in dipilih" :key="'badge-' + kursi">
                                    <span class="px-2 py-1 bg-blue-600 text-white rounded-lg text-xs font-bold"
                                        x-text="kursi">
                                    </span>
                                </template>
                            </div>
                        ...
                        <div x-show="dipilih.length === 0"
                            class="text-sm text-slate-400 text-center py-8 flex flex-col items-center gap-2">
                            ...
                            <template x-for="(kursi, i) in dipilih" :key="'form-' + kursi">
                                <div class="flex items-center gap-3">
                                    <div class="w-12 h-10 bg-blue-600 text-white rounded-xl flex items-center justify-center text-xs font-bold flex-shrink-0"
                                        x-text="kursi"></div>
                                    <input type="text" :id="'nama_penumpang_pergi_' + i"
                                        x-model="namaPenumpangPergi[i]" placeholder="Nama penumpang" required
                            ...
                            <template x-for="(kursi, i) in dipilihPulang" :key="'pform-' + kursi">
                                <div class="flex items-center gap-3">
                                    <div class="w-12 h-10 bg-purple-600 text-white rounded-xl flex items-center justify-center text-xs font-bold flex-shrink-0"
                                        x-text="kursi"></div>
                                    <input type="text" x-model="namaPenumpangPulang[i]" placeholder="Nama penumpang"
                                        required
                    ...
                    <button type="button" @click="lanjutKePembayaran()" :disabled="!bisaLanjut()"
                        :class="bisaLanjut() ? 'bg-blue-600 hover:bg-blue-700 cursor-pointer' :
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div class="lg:col-span-2 space-y-4">
                                ...
                                <div class="px-4 py-3 space-y-2">
                                    <template x-for="(kursi, i) in dipilih" :key="'sum-' + kursi">
                                        <div class="flex items-center justify-between text-sm">
                                            <div class="flex items-center gap-2">
                                                <span
                                                    class="font-mono font-bold text-slate-700 bg-blue-50 px-2 py-0.5 rounded text-xs"
                                                    x-text="kursi"></span>
                                                <span class="text-slate-600" x-text="namaPenumpangPergi[i] || '—'"></span>
                                            </div>
                                            <span class="text-slate-700 font-medium">Rp
                                                {{ number_format($jadwal->harga_tiket, 0, ',', '.') }}</span>
                                        </div>
                                    </template>
                                </div>
                            </div>
                                ...
                                <div class="px-4 py-3 space-y-2">
                                    <template x-for="(kursi, i) in dipilihPulang" :key="'psum-' + kursi">
                                        <div class="flex items-center justify-between text-sm">
                                            <div class="flex items-center gap-2">
                                                <span
                                                    class="font-mono font-bold text-purple-700 bg-purple-50 px-2 py-0.5 rounded text-xs"
                                                    x-text="kursi"></span>
                                                <span class="text-slate-600"
                                                    x-text="namaPenumpangPulang[i] || '—'"></span>
                                            </div>
                        ...
                        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
                            ...
                            <div class="space-y-3">
                                {{-- CASH --}}
                                <label class="flex items-start gap-4 p-4 border-2 rounded-xl cursor-pointer transition-all"
                                    :class="metodePembayaran === 'Cash' ? 'border-green-500 bg-green-50' :
                                        'border-slate-200 hover:border-green-300 hover:bg-green-50/50'">
                                    <input type="radio" name="metode_pembayaran" value="Cash"
                                        x-model="metodePembayaran" required
                                        class="mt-1 text-green-600 w-4 h-4 flex-shrink-0">
                                    <div class="flex-1">
                                        <div class="flex items-center gap-2">
                                            <span class="text-2xl">💵</span>
                                ...
                                {{-- E-WALLET --}}
                                <label class="flex items-start gap-4 p-4 border-2 rounded-xl cursor-pointer transition-all"
                                    :class="metodePembayaran === 'E-Wallet' ? 'border-purple-500 bg-purple-50' :
                                        'border-slate-200 hover:border-purple-300 hover:bg-purple-50/50'">
                                    <input type="radio" name="metode_pembayaran" value="E-Wallet"
                                        x-model="metodePembayaran" required
                                        class="mt-1 text-purple-600 w-4 h-4 flex-shrink-0">
                                    <div class="flex-1">
                                        <div class="flex items-center gap-2">
                                            <span class="text-2xl">📱</span>
                                        ...
                                        <p class="text-xs text-slate-500 mt-1">GoPay, OVO, DANA, ShopeePay — transfer ke
                                            nomor berikut.</p>
                                        {{-- Instruksi E-Wallet --}}
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="bg-slate-800 text-white rounded-2xl p-6 sticky top-24">
                            ...
                            <div class="space-y-2 text-sm mb-4">
                                ...
                                <div x-show="isRoundTrip && dipilihPulang.length > 0" x-cloak
                                    ...
                                    <div class="flex justify-between text-slate-300">
                                        ...
                                        <span
                                            x-text="dipilihPulang.length + ' × Rp ' + hargaPulang.toLocaleString('id-ID')"></span>
                                    </div>
                                    <div class="flex justify-between text-slate-300 font-medium">
                                        <span></span>
                                        <span
                                            x-text="'Rp ' + (dipilihPulang.length * hargaPulang).toLocaleString('id-ID')"></span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <button type="submit" :disabled="!metodePembayaran"
