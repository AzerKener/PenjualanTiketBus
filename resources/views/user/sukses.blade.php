...
<div class="max-w-2xl mx-auto px-4 py-12">
    ...
    <div class="text-center mb-8">
        ...
        <h1 class="text-2xl font-extrabold text-slate-800">Pemesanan Berhasil!</h1>
        <p class="text-slate-500 mt-2">Terima kasih, {{ $pemesanan->nama_pemesan }}. Tiket Anda telah dikonfirmasi.</p>
    </div>
    ...
    <div class="bg-white rounded-3xl border border-slate-200 shadow-lg overflow-hidden mb-4">
        ...
        <div class="bg-gradient-to-r from-blue-600 to-blue-700 px-6 py-5 flex items-center justify-between">
            ...
            @if($pemesanan->status_pembayaran === 'lunas')
                <span class="bg-green-400 text-green-900 px-4 py-2 rounded-xl text-sm font-bold">LUNAS</span>
            @else
                <span class="bg-amber-400 text-amber-900 px-4 py-2 rounded-xl text-sm font-bold">PENDING</span>
            @endif
        </div>
        ...
        <div class="p-6 border-b border-slate-100">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 gap-4 mt-5 text-sm">
                <div>
                    ...
                    <p class="font-semibold text-slate-700">{{ \Carbon\Carbon::parse($pemesanan->jadwal->tanggal_berangkat)->isoFormat('D MMM Y') }}</p>
                </div>
                <div>
        ...
        <div class="p-6 border-b border-slate-100">
            <p class="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3">Penumpang
                @if($pemesanan->is_round_trip)<span class="text-blue-500 ml-1">(Tiket Pergi)</span>@endif
            </p>
            @php $penumpangPergi = $pemesanan->penumpangs->where('jadwal_id', $pemesanan->jadwal_id); @endphp
            <div class="space-y-2">
                @foreach($penumpangPergi as $p)
                <div class="flex items-center gap-3">
                    <span class="font-mono font-bold text-slate-700 bg-slate-100 px-2.5 py-1 rounded-lg text-sm">{{ $p->nomor_kursi }}</span>
            ...
            <div class="mt-4 pt-4 border-t border-slate-100">
                <p class="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3">Penumpang (Pulang: {{ $pemesanan->jadwalPulang->rute->asal }} → {{ $pemesanan->jadwalPulang->rute->tujuan }}, {{ \Carbon\Carbon::parse($pemesanan->jadwalPulang->tanggal_berangkat)->isoFormat('D MMM') }})</p>
                <div class="space-y-2">
                    @foreach($penumpangPulang as $p)
                    <div class="flex items-center gap-3">
                        <span class="font-mono font-bold text-purple-700 bg-purple-100 px-2.5 py-1 rounded-lg text-sm">{{ $p->nomor_kursi }}</span>
                        <span class="text-slate-700">{{ $p->nama_penumpang }}</span>
                    </div>
                    @endforeach
                </div>
        ...
        <div class="p-6">
            <div class="flex items-center justify-between">
                ...
                <div class="text-right">
                    ...
                    <p class="text-2xl font-extrabold text-green-600">Rp {{ number_format($pemesanan->total_bayar, 0, ',', '.') }}</p>
                </div>
            </div>
