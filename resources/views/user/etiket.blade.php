...
<div class="max-w-2xl mx-auto px-4 py-8">
    <div class="flex items-center justify-between mb-4 print:hidden">
        <a href="{{ route('user.riwayat') }}" class="flex items-center gap-1.5 text-sm text-slate-600 hover:text-blue-600">
            ← Riwayat Pemesanan
        </a>
    ...
    <div class="bg-white rounded-3xl border-2 border-slate-200 shadow-xl overflow-hidden" id="eticket-content">
        ...
        <div class="bg-gradient-to-r from-blue-700 to-blue-600 px-8 py-6 flex items-center justify-between">
            ...
            <div class="text-right">
                ...
                <p class="text-white text-2xl font-extrabold tracking-widest">#{{ str_pad($pemesanan->id, 6, '0', STR_PAD_LEFT) }}</p>
            </div>
        </div>
        ...
        <div class="px-8 py-6">
            <div class="flex items-center gap-4 mb-6">
                <div class="text-center">
                    ...
                    <p class="text-sm text-slate-500 font-semibold mt-0.5">{{ substr($pemesanan->jadwal->waktu_berangkat, 0, 5) }} WIB</p>
                </div>
                <div class="flex-1 flex flex-col items-center gap-0.5">
                    ...
                    <span class="text-xs text-blue-600 font-semibold bg-blue-50 px-2 py-0.5 rounded">PERGI-PULANG</span>
                    @endif
                </div>
                <div class="text-center">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-6">
                <div class="bg-slate-50 rounded-xl p-3">
                    ...
                    <p class="font-bold text-slate-700 mt-0.5 text-sm">{{ \Carbon\Carbon::parse($pemesanan->jadwal->tanggal_berangkat)->isoFormat('D MMMM Y') }}</p>
                </div>
                <div class="bg-slate-50 rounded-xl p-3">
                    <p class="text-xs text-slate-400 font-medium uppercase">Bus</p>
                    <p class="font-bold text-slate-700 font-mono mt-0.5 text-sm">{{ $pemesanan->jadwal->bus->nomor_polisi }}</p>
                ...
                <div class="bg-slate-50 rounded-xl p-3">
                    ...
                    @if($pemesanan->status_pembayaran === 'lunas')
                        <span class="inline-flex items-center mt-0.5 px-2.5 py-0.5 rounded-full text-xs font-bold bg-green-100 text-green-700">✓ LUNAS</span>
                    @else
                        <span class="inline-flex items-center mt-0.5 px-2.5 py-0.5 rounded-full text-xs font-bold bg-amber-100 text-amber-700">PENDING</span>
                    @endif
                </div>
            </div>
            ...
            <div class="border border-slate-200 rounded-2xl overflow-hidden mb-4">
                <div class="bg-slate-50 px-4 py-2.5 border-b border-slate-200">
                    <p class="text-xs font-bold text-slate-500 uppercase tracking-wider">Daftar Kursi & Penumpang (Pergi)</p>
                </div>
                <div class="divide-y divide-slate-100">
                    @foreach($penumpangPergi as $pnp)
                    <div class="flex items-center gap-3 px-4 py-2.5">
                        <span class="font-mono font-extrabold text-blue-700 bg-blue-50 px-3 py-1 rounded-lg text-sm">{{ $pnp->nomor_kursi }}</span>
            ...
            <div class="border border-purple-200 rounded-2xl overflow-hidden mb-4">
                <div class="bg-purple-50 px-4 py-2.5 border-b border-purple-200">
                    <p class="text-xs font-bold text-purple-500 uppercase tracking-wider">
                        Perjalanan Pulang: {{ $pemesanan->jadwalPulang->rute->asal }} → {{ $pemesanan->jadwalPulang->rute->tujuan }}
                ...
                <div class="divide-y divide-slate-100">
                    ...
                    <div class="flex items-center gap-3 px-4 py-2.5">
                        <span class="font-mono font-extrabold text-purple-700 bg-purple-50 px-3 py-1 rounded-lg text-sm">{{ $pnp->nomor_kursi }}</span>
                        <span class="text-slate-700 font-medium text-sm">{{ $pnp->nama_penumpang }}</span>
                    </div>
                    @endforeach
                </div>
            ...
            {{-- Total --}}
            <div class="bg-gradient-to-r from-slate-800 to-slate-700 text-white rounded-2xl p-4 flex items-center justify-between">
                <div>
                ...
                <div class="text-right text-xs text-slate-400">
                    <p>Tanggal Pesan:</p>
                    <p class="text-white font-semibold">{{ \Carbon\Carbon::parse($pemesanan->tanggal_transaksi)->isoFormat('D MMM Y HH:mm') }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="border-t border-slate-200 px-8 py-4 bg-slate-50 text-center">
            <p class="text-xs text-slate-400">Harap tiba di pool keberangkatan minimal 30 menit sebelum jadwal. Tunjukkan e-tiket ini kepada petugas.</p>
        </div>
    </div>
</div>
...
@media print {
    ...
    #eticket-content { box-shadow: none; border: 1px solid #ccc; }
}
</style>
@endpush
@endsection
